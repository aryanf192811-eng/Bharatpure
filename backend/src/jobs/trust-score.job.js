...
/**
 * Computes and persists fpo_trust_scores + buyer_reliability_scores for every FPO/buyer "with
 * activity" (has at least one non-draft batch / at least one order), per BHARATPURE-DB.md table
 * 26's formula (given verbatim) and this task's own analogous buyer formula (no formula was
 * given for buyers — see the research doc for why weights were chosen the way they were).
 *
  ...
  try {
    ...
    for (const { fpo_id: fpoId } of fpoResult.rows) {
      ...
      await client.query(
        ...
        [fpoId, fulfillmentRate, qualityConsistency, onTimeDeliveryRate, disputeRate, buyerRatingAvg, Number(m.total_batches), computedScore],
      );
      // eslint-disable-next-line no-await-in-loop
    ...
    return { fpo_scores_computed: fpoScoresComputed, buyer_scores_computed: buyerScoresComputed, duration_ms: durationMs };
  } catch (err) {
